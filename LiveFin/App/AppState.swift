...
@Published var canManageRecordings: Bool = true
...
#endif
Task {
await self.refreshUserProfileInfoAndImage()
await self.fetchUserPermissions()
}
...
private func resetState() {
    ...
    JellyfinV12Interceptor.authHeader = nil
...
@MainActor
func fetchUserPermissions() async {
    ...
    do {
        ...
        struct UserPolicyDto: Decodable {
            let EnableLiveTvManagement: Bool?
        }
        struct UserDetailDto: Decodable {
            let Policy: UserPolicyDto?
        }
        
        if let detail = try? JSONDecoder().decode(UserDetailDto.self, from: data) {
            self.canManageRecordings = detail.Policy?.EnableLiveTvManagement ?? false
        }
    } catch {
        print("[Permissions] User policy fetch failed: \(error)")
    }
}
