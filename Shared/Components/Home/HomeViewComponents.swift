...
struct HorizontalChannelsRow: View {
    ...
    var body: some View {
        ScrollView(.horizontal, showsIndicators: false) {
                ...
                ForEach(Array(channels.enumerated()), id: \.offset) { index, channel in
                    NavigationLink(destination: ChannelDetailView(channel: channel.asLiveDto(baseURL: appState.serverURL))) {
                        ZStack(alignment: .topTrailing) {
                            ZStack {
                                // Background blur of the channel image
                                ChannelImageView(baseUrl: appState.serverURL, apiKey: appState.apiKey, channelId: channel.id)
                                    .frame(width: 84, height: 84)
                                    .blur(radius: 12)
                                    .opacity(0.8)
                                
                                // NOTE: the original `#available(iOS 26.0, *)` check only evaluates
                                // the iOS clause on an iOS build; on a tvOS build the iOS clause is
                                // ignored and the `*` wildcard makes this true on ANY tvOS version,
                                // so it would try to call glassEffect on tvOS 17/18 too. List both
                                // platforms explicitly so each is gated on its own OS 26 minimum.
            ...
            .padding(.horizontal)
            .padding(.vertical, 16) // Increased top-bottom spacing
            .frame(minHeight: 110)
        }
    }
}
...
struct ProgramCard: View {
    ...
    var body: some View {
        ...
        VStack(alignment: .leading, spacing: 6) {
            ZStack(alignment: .bottomLeading) {
                ...
                if let url = URL(string: "\(base)/Items/\(program.id)/Images/Primary?maxWidth=400&ApiKey=\(appState.apiKey)") {
                    CachedAsyncImage(url: url) { phase in
                        ...
                        case .success(let image):
                            ZStack {
                                // 1. Blurred background filling the box with native material overlay
                                ZStack {
                                    image
                                        .resizable()
                ...
                // Show Recording Status Bar, fallback to progress ratio if not recording
                if isRecording {
    ...
    private var imageWidth: CGFloat {
        ...
        case .landscapeLarge: return 350 // Increased size even more for On Now
    ...
    private var imageHeight: CGFloat {
        ...
        case .landscapeLarge: return 197 // 16:9 ratio based on 350 width
...
struct LibraryItemCard: View {
    ...
    private func renderImage(image: Image) -> some View {
        ZStack {
            // Blurred iOS background with ultra thin material overlay
            ZStack {
                image
                    .resizable()
                    .aspectRatio(contentMode: .fill)
