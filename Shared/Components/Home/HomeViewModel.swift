...
//  LiveFin
//
//  Extracted from HomeView.swift so it can be shared with the tvOS target
//  without pulling in iOS-only view code (HomeView's body calls ToolbarView(),
//  which hasn't been reviewed for tvOS compatibility).
//
//  IMPORTANT: after adding this file, delete the "MARK: - ViewModel" and
//  "MARK: - Models" sections (HomeViewModel, JFChannel, and its extension)
//  from HomeView.swift — leave only the HomeView struct there. Having the
//  same type defined in two files in the same (iOS) target is a compile
