...
struct JFPlaybackInfoService {
    ...
    private static func buildDeviceProfile(maxBitrate: Int?, isLiveTV: Bool) -> [String: Any] {
        ...
        // tvOS (Apple TV) is almost always wired/strong Wi-Fi and HDMI-connected to an
        // AVR/soundbar, so it can pass through multichannel bitstream audio that a phone
        // or iPad never will. iOS is capped more conservatively for AirPlay/Bluetooth/
        // built-in-speaker playback.
        #if os(tvOS)
        let profileName = "LiveFin tvOS"
        let defaultBitrate = 200_000_000
        let directPlayAudioCodecs = "aac,ac3,eac3,truehd,dts,dca,mp3,alac,flac"
        let hlsAudioCodecs = "aac,ac3,eac3,truehd,dts,dca,mp3,flac"
        let transcodeAudioCodecs = "aac,ac3,eac3,truehd,dts,dca,mp3,alac,flac"
        let maxAudioChannels = "8"
        #else
        let profileName = "LiveFin iOS"
        let defaultBitrate = 140_000_000
        let directPlayAudioCodecs = "aac,ac3,eac3,mp3,alac,flac"
        let hlsAudioCodecs = "aac,ac3,eac3,mp3,flac"
        let transcodeAudioCodecs = "aac,ac3,mp3,alac,flac,eac3"
        let maxAudioChannels = "6"
        #endif

        profile["Name"] = profileName
        profile["MaxStreamingBitrate"] = maxBitrate ?? defaultBitrate
        ...
        // ts is offered as a transcode target for both Live TV and library (VOD) content,
        // on both platforms — just deprioritized behind mp4 for library since most servers/
        // clients handle fMP4-in-HLS more predictably for on-demand seeking.
        if isLiveTV {
        ...
        profile["CodecProfiles"] = [[
            ...
            "Type": "Audio", "Codec": "aac", "Conditions": [[
                "Condition": "LessThanEqual", "Property": "AudioChannels", "Value": maxAudioChannels
            ]]
        ]]
