...
struct JFProgram: Identifiable, Hashable {
    ...
    init?(json: [String: Any]) {
        ...
        self.isPremiere = json["IsPremiere"] as? Bool
        self.isNew = json["IsNew"] as? Bool
...
final class ProgramViewModel: ObservableObject {
    ...
    @Published var displayedUpcoming: [JFProgram] = []
    ...
    private var seenUpcomingKeys: Set<String> = []
    ...
    func fetchNextUpcomingPage() async {
        guard !isLoadingMoreUpcoming && hasMoreUpcoming else { return }
        isLoadingMoreUpcoming = true
        
        let now = Date()
        guard let windowStart = Calendar.current.date(byAdding: .day, value: upcomingDaysOffset, to: now),
              let windowEnd = Calendar.current.date(byAdding: .day, value: upcomingDaysOffset + 14, to: now) else {
            isLoadingMoreUpcoming = false
            return
        }
        
        let iso = ISO8601DateFormatter(); iso.formatOptions = [.withInternetDateTime]
        let minStart = iso.string(from: windowStart)
        let maxStart = iso.string(from: windowEnd)
        ...
        let matched = allFound.filter { p in
            ...
            if p.channelId == effectiveChannelId {
                ...
                } else if p.name.caseInsensitiveCompare(program.name) == .orderedSame {
                    return true
                }
            }
        ...
        var newResults: [JFProgram] = []
        for p in matched.sorted(by: { ($0.startDate ?? .distantFuture) < ($1.startDate ?? .distantFuture) }) {
            guard let start = p.startDate else { continue }
            let key = p.id + "|" + (p.channelId ?? "") + "|" + String(Int(start.timeIntervalSince1970))
            if seenUpcomingKeys.insert(key).inserted {
                newResults.append(p)
            }
        }
    ...
    var body: some View {
        ScrollView {
            LazyVStack(spacing: 0) {
                ForEach(viewModel.displayedUpcoming) { program in
                    UpcomingProgramRow(
                        program: program,
                        referenceName: viewModel.program.name,
                        referenceStart: viewModel.program.startDate
                    )
                    .onAppear {
                        if program.id == viewModel.displayedUpcoming.last?.id {
                            Task { await viewModel.fetchNextUpcomingPage() }
                        }
                    }
                    Divider().padding(.leading, 8)
                }
